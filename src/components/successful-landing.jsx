...
class SuccessfulLanding extends Component {
    constructor(props) {
        super(props);
        this.state = {
            items: [],
            url1: 'https://api.spaceXdata.com/v3/launches?limit=100&launch_success=',
            url2: '&land_success=',
        };
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if(this.props.year !== prevState.year) {
            let updatedUrl = this.state.url1+ this.props.year + this.state.url2 + this.props.year;
                fetch(updatedUrl)
                .then((res) => res.json())
